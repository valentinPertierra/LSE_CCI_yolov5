...
# Funciones para hacer inferencias con modelo de mediapipe
...
id2class = {'cintas': 0, 'corte': 1, 'deformacion': 2, 'grumo': 3, 'mancha': 4, 'otros': 5, 'perforacion chica': 6, 'perforacion grande': 7, 'perforacion mediana': 8, 'protuberancia': 9, 'background': 0}
...
def run(
    ...
    # Configuracion de modelo de mediapipe
    base_options = python.BaseOptions(model_asset_path=mp_model)
    options = vision.ObjectDetectorOptions(base_options=base_options,running_mode=vision.RunningMode.IMAGE, score_threshold=conf_thres,max_results=max_det)

    mp_detector = vision.ObjectDetector.create_from_options(options)
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Utilizo el modelo de mediapipe para realizar las inferencias
        preds = []
        for imgPath in paths:
            cv_mat = cv2.imread(imgPath, cv2.IMREAD_COLOR)
            mpImage = mep.Image(image_format=mep.ImageFormat.SRGB, data=cv_mat)
            
            detres = mp_detector.detect(mpImage)
            
            mppreds = []
            for deteccion in detres.detections:
                bbox = deteccion.bounding_box
                category = deteccion.categories[0]
                
                mppreds.append([bbox.origin_x, bbox.origin_y, bbox.origin_x+bbox.width,bbox.origin_y+bbox.height,category.score, id2class[category.category_name]])
                
            # Escalo los bbox de media pipe para que coincida con las que obtendria con yolo
            mptensor = torch.tensor(mppreds, dtype=torch.float32)
            mptensor[..., :4] *= 0.5
            mptensor[..., [0,2]] += 16
            mptensor[..., [1,3]] += 12
            
            preds.append(mptensor)
    ...
    if len(stats) and stats[0].any():
        tp, fp, p, r, f1, ap, ap_class,th_mF1 = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
    ...
    print(f"Th max F1 {th_mF1:.3f}")
    
    # Guardo las metricas resultantes en un archivo 
    save_metrics = []
    save_metrics.append({"Class": "all", "Images": int(seen), "Instances":  int(nt.sum()), "P": float(mp), "R": float(mr), "mAP50": float(map50), "mAP50-95": float(map)})
    for i, c in enumerate(ap_class):
        save_metrics.append({"Class": names[c], "Images": int(seen), "Instances":  int(nt[c]), "P": float(p[i]), "R": float(r[i]), "mAP50": float(ap50[i]), "mAP50-95": float(ap[i])})
    
    with open(save_dir / 'metrics.json', "w") as f:
        json.dump(save_metrics, f)
        
    with open(save_dir / 'th_f1.json', "w") as f:
        json.dump({'th':th_mF1}, f)
...
def parse_opt():
    ...
    parser.add_argument("--mp_model", type=str, default=ROOT / "model.tflite", help="mediapipe model path(s)")
